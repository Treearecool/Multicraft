...
// -------------------------------------------------------------------------
	...
	$icon_directory = 'images/net2ftp/mime';
	if (!@file_exists(Theme::themeFilePath($icon_directory.'/'.$icon)))
		$icon = 'mime';
	$icon_directory = Theme::themeFile($icon_directory);
	...
	elseif ($net2ftp_settings["fix_png"] == "yes" && $net2ftp_globals["browser_agent"] == "IE" && $net2ftp_globals["browser_platform"] == "Win" && ($net2ftp_globals["browser_version"] == "5.5" || $net2ftp_globals["browser_version"] == "6.0" || $net2ftp_globals["browser_version"] == "7.0")) { 
		$mime["mime_icon"] = "<img src=\"$icon_directory/spacer.gif\" alt=\"icon\" style=\"border: 0px; filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='$icon_directory/$icon', sizingMethod='scale')\" />\n"; 
	}
	else { 
		$mime["mime_icon"] = "<img src=\"$icon_directory/$icon\"      alt=\"icon\" style=\"border: 0px;\" />\n"; 
	}
...
// -------------------------------------------------------------------------
	...
	if (!$a)
	{
		echo '<input type="submit" class="btn btn-primary" value="'.$alt.'" onclick="'.$onclick.'; return false" />';
		return;
	}
